...
bool ICM42688P::begin(ICM_BUS busType, int8_t csPin, uint32_t freq, uint8_t i2cAddr, int8_t sckSclPin, int8_t misoSdaPin, int8_t mosiPin) {
  ...
  if (_bus == BUS_SPI) {
    if (_csPin != -1) { pinMode(_csPin, OUTPUT); digitalWrite(_csPin, HIGH); }
    #if defined(ESP32)
      if (sckSclPin != -1 && misoSdaPin != -1 && mosiPin != -1) { SPI.begin(sckSclPin, misoSdaPin, mosiPin, _csPin); } 
      else { SPI.begin(); }
    #elif defined(ARDUINO_ARCH_RP2040) || defined(ARDUINO_ARCH_RP2350)
      if (sckSclPin != -1 && misoSdaPin != -1 && mosiPin != -1) { SPI.setSCK(sckSclPin); SPI.setRX(misoSdaPin); SPI.setTX(mosiPin); }
      SPI.begin();
    #else
      SPI.begin(); 
    #endif
    ...
    #if defined(ESP32) 
      if (sckSclPin != -1 && misoSdaPin != -1) { Wire.setPins(misoSdaPin, sckSclPin); }
    #elif defined(ARDUINO_ARCH_RP2040) || defined(ARDUINO_ARCH_RP2350)
      if (sckSclPin != -1 && misoSdaPin != -1) { Wire.setSDA(misoSdaPin); Wire.setSCL(sckSclPin); }
    #endif
    Wire.begin(); Wire.setClock(_spiFreq);
    Wire.beginTransmission(_i2cAddr);
    if (Wire.endTransmission() != 0) { _i2cAddr = (_i2cAddr == ICM_ADDR_PRIMARY) ? ICM_ADDR_SECONDARY : ICM_ADDR_PRIMARY; }
  }
...
ICM_FIFO_MODE ICM42688P::getFIFOMode() { return _fifoMode; }
...
void ICM42688P::enforceBandwidthLimit() {
  ...
  else if (_fifoMode == FIFO_16BIT) bytesPerRead = 16; 
  else if (_fifoMode == FIFO_20BIT_HIRES) bytesPerRead = 20; 
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if ((bitsPerRead * targetHz) <= maxAllowedBps) {
      safeODR = odrList[i];
      break;
    }
  }
...
bool ICM42688P::readIMU(float &ax, float &ay, float &az, float &gx, float &gy, float &gz) {
  if (_fifoMode == FIFO_NONE) return readSensorData(ax, ay, az, gx, gy, gz);
  else if (_fifoMode == FIFO_16BIT) return readHardwareFIFO(ax, ay, az, gx, gy, gz);
  else return readHardwareFIFOHires(ax, ay, az, gx, gy, gz);
}
...
void ICM42688P::flushFIFO() {
  writeRegister(ICM42688_REG_SIGNAL_PATH_RESET, 0x02); // FIFO Flush bit
}
...
bool ICM42688P::readHardwareFIFOHires(float& ax, float& ay, float& az, float& gx, float& gy, float& gz) {
  ...
  // 20-bit Parsování - zcela opraveno a otestováno
  int32_t rawAx = (int32_t)((buffer[1] << 12) | (buffer[2] << 4) | (buffer[17] >> 4));
  if (rawAx & 0x80000) rawAx |= 0xFFF00000;
